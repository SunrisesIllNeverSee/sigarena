---
type: Plan
title: SigRank SEO + GEO — Full Implementation Plan
description: Full SEO and Generative Engine Optimization plan for SigRank/signalaf.com. Source from PR #12 on sigrank-app.
tags: [seo, geo, sigrank, signalaf, discoverability, implementation]
timestamp: 2026-06-30T03:00:00Z
---

# SigRank SEO + GEO — Full Implementation Plan

> **Source:** PR #12 on `sigrank-app` (branch `claude/sigrank-mcp-review-vghe3d`, DRAFT).
> Pulled into RNS 2026-06-28 for implementation planning.
...
## STATUS (audited 2026-06-28, verified live 2026-06-29)

| Phase | Status | Commit | Notes |
|-------|--------|--------|-------|
| 1 — OG image raster fix | ✅ DONE | `d5d814b` + `6e877ed` | `public/og.png` (1200×630). Pre-existing. |
| 2 — JSON-LD structured data | ✅ DONE | `bdd326a` + `4b4fca0` | Org+WebSite (layout), ItemList (board), ProfilePage (user), Breadcrumb+DefinedTerm (5 wiki pages), Breadcrumb (hall, about, wiki hub). New: `components/seo/JsonLd.tsx`, `lib/jsonld.ts`. |
| 3 — llms.txt | ✅ DONE | `116beaa` | `app/llms.txt/route.ts` + sitemap entry. Live at signalaf.com/llms.txt. |
| 4 — Per-page dynamic OG | ✅ DONE | `55b9138` | `app/user/[codename]/opengraph-image.tsx` + `app/board/[window]/opengraph-image.tsx`. Both live (200, image/png, immutable cache). |
| 5 — npm keywords + GitHub topics | ✅ DONE | `e861e81` (mcp) + `gh repo edit` | 15 npm keywords. GitHub topics updated on both repos. |

**PR #12 closed** — plan fully implemented. Branch `claude/sigrank-mcp-review-vghe3d` deleted.
