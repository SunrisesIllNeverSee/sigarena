...
cryptographically authenticated provenance embedded directly in the PNG file.
This allows users and AI agents to distinguish genuine SigRank-issued share cards
from forged badge or ranking images.

### How it works

1. When a share-card image is generated (e.g., via /share/mcp), a C2PA manifest
   is embedded into the PNG before delivery.
2. The manifest includes:
   - Creator: SigRank SignalAF
   - Producing tool: SigEconomy MCP Share Card Generator
   - Issuance timestamp
   - Actions performed (c2pa.created)
   - Hard binding (SHA-256 hash of the image bytes)
3. The manifest is signed with an ECDSA P-256 key (ES256 algorithm).
4. The signing certificate is included in the COSE_Sign1 protected header.

### Verification

To verify a share card's provenance:
- Upload the image to https://contentcredentials.org/verify
- Or use any C2PA-compatible verifier

### Current limitations

- The signing certificate is currently self-signed. Some C2PA verifiers may flag
  self-signed credentials as untrusted. This does not affect the cryptographic
  integrity of the manifest — it only affects the trust display.
- Upgrade path: obtain a CA-issued C2PA-approved certificate from a recognized
  certificate authority (e.g., Adobe C2PA cert, Truepic).

### MCP integration

The MCP server card (/.well-known/mcp) includes a \`contentCredentials\` field
in its \`_meta\` section advertising C2PA support. Tool results include a
\`content_credentials\` field in the \`share\` object pointing to the verification URL.
